/*
 * Problem counting_rooms (cses/graph/counting_rooms)
 * Create time: Sun 06 Dec 2020 @ 15:45 (PST)
 * Accept time: Sun 06 Dec 2020 @ 15:49 (PST)
 *
 */
...
void setIO(const std::string &name = "counting_rooms")
{
...
void flood(int y, int x, int n)
{
    if (buf[y][x] != '.') return;
    buf[y][x] = 'x';
    for (int d=0; d<4; ++d)
        flood(y+_y[d], x+_x[d], n);
}

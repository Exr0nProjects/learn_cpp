...
const int maxn = 2000;	// should technically be `maxa`
const ll inf = 1e14;
int n, m;
...
ll solve(int l, int r) {
	...
	if (pl > pr) return inf;	// should never happen
	ll ans = sum[pr] - sum[pl];	// expected cost of the full range contained by l+1..r
	ans += f[l][1]; ans += f[r][0];	// plus the left and right bits
	return ans;
}
...
ll calc(int l, int r) { // expected cost * range of the range inc l inc r
	if (r <= a[1]) return 0;
	ll ans = inf;
	for (int i = 1; i <= l - 1; ++i)	// try reducing range by `i` for all i w/o going under	
		ans = min(ans, solve(l - i, r - i) + g[i]*(r - l + 1));	// expected cost of new range * cost of operation
	return ans;
}
...
int main() {
	...
	while (T--) {
		...
		// no need to check for using a lower op before a higher one, because order doesn't matter
		// 	because both cost and effectiveness add up
		for (int i = 1; i <= m; ++i) {			// for each operation
			int v, c; scanf("%d%d", &v, &c);
			for (int j = c; j <= a[n]; ++j)		// for every `j` we can do this operation on
			    //  ^ this allows chaining of multiple of the same op, because it gos from down to up

...
int main()
{
    ...
    TRAV(p, points)
    {
        ...
        auto &v = vert[p.F];
        printf("idx.F = %d\n",idx.S);
        ll hsum_l = idx.F > 0 ? ((idx.F*p.F) - (h[idx.F-1])) : 0;
        ll hsum_r = ((h[h.size()-1]-h[idx.F])-((h.size()-1-idx.F)*p.F));

        ll vsum_l = idx.S > 0 ? ((idx.S*p.S) - (v[idx.S-1])) : 0;
        ll vsum_r = ((v[v.size()-1]-v[idx.S])-((v.size()-1-idx.S)*p.S));

        printf("    point (%lld %lld) hsum %lld+%lld vsum %lld+%lld\n", p.F, p.S,
                hsum_l, hsum_r, vsum_l, vsum_r);

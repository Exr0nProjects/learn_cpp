/*
 * Problem dinic1 (../algorithms/graphs/network_flow/dinic1)
 * Create time: Thu 15 Oct 2020 @ 18:19 (PDT)
 * Accept time: [!meta:end!]
 *
 */

#include <iostream>
#include <cstring>
#include <algorithm>
#include <utility>
#include <queue>

#define ll long long
#define dl double

#define gpq(T) priority_queue<T, deque<T>, greater<T> >
#define CMA , // https://stackoverflow.com/q/13842468/10372825
#define pii pair<int, int>
#define pb push_back
#define mp make_pair
#define f first
#define s second
#define lb(x) ((x)&-(x))

#define F(i,b) for (ll i=1; i<=(b); ++i)
#define R(i,b) for (ll i=(b); i>=1; --i)
//struct Edge { int u, v, n; } eg[MX]; int hd[MX], ecnt=0;

void setIO(const std::string &name = "dinic1")
{
    //ios_base::sync_with_stdio(0); cin.tie(0);
    if (fopen((name + ".in").c_str(), "r") != 0)
        freopen((name + ".in").c_str(), "r", stdin),
        freopen((name + ".out").c_str(), "w+", stdout);
}
#define _gc getchar_unlocked
inline bool sc(ll &n)
{
    int neg = 1;
    register char c;
    do c = _gc(); while (isspace(c));
    if (c == '-') neg = -1, c = _gc();
    for (n=0; c >= '0' && c <= '9'; c=_gc())
        (n *= 10) += (c-'0');
    n *= neg;
    return c != EOF;
}
inline ll sc() { ll x; sc(x); return x; }
#define _ilb inline bool
_ilb sc(int&a){ll x;bool b=sc(x);a=x;return b;}
_ilb sc(int&a,int&b){return sc(a)&&sc(b);}
_ilb sc(int&a,int&b,int&c){return sc(a,b)&&sc(c);}
_ilb sc(int&a,int&b,int&c,int&d){return sc(a,b)&&sc(c,d);}
_ilb sc(ll&a,ll&b){return sc(a)&&sc(b);}
_ilb sc(ll&a,ll&b,ll&c){return sc(a,b)&&sc(c);}
_ilb sc(ll&a,ll&b,ll&c,ll&d){return sc(a,b)&&sc(c,d);}
#define gcd(_a, _b) ({auto a=_a;auto\
    b=_b;while(b)(a)%=(b),(a)^=(b)^=(a)^=(b);a;})

using namespace std;
const int MX = 1e5+11;

int N, M, S, T, dep[MX], cap[MX][MX];

bool kdep()
{
    F(i, N) { F(j, N) printf("%3d", cap[i][j]); printf("\n"); }
    memset(dep, 0, sizeof dep);
    dep[S] = 1;
    queue<int> q;
    q.push(S);
    bool flag=0;
    while (!q.empty())
    {
        int c = q.front(); q.pop();
        printf("c = %d, %d\n", c, dep[c]);
        if (c == T) flag = 1;
        for (int n=1; n<=N; ++n)
            if (!dep[n] && cap[c][n])   // FIX: don't revisit previous nodes (check that dep = 0)
            dep[n] = dep[c]+1, q.push(n);
    }
    return flag;
}

int aug(int c, int p, int mn)
{
    if (!mn || c == T) return cap[p][c] -= mn, mn;
    printf("%d < %d w/ %d\n", c, p, mn);
    int flo=0;
    for (int n=1; n<=N; ++n) if (dep[n] -1 == dep[c])
        if (int g=aug(n, c, min(mn, cap[c][n])))
        {
            flo += g;
            cap[p][c] -= g;
            cap[c][p] += g;
            mn -= g;
        }
    return flo;
}

int main()
{
    sc(N, M, S, T);
    for (int i=1; i<=M; ++i)
    {
        int u, v, w; sc(u, v, w);
        cap[u][v] = w;
    }
    cap[0][S] = 1e9;

    int flo=0;
    while (kdep())
        flo += aug(S, 0, 1e9);
    printf("%d\n", flo);
}


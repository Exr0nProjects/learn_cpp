...
DP13 percentage to raise | 1d array, x-branch tree | `dp[i]` = min leaf nodes needed to convince `i` | `dp[i] = (min T%){dp[k] : k ∈ i}`
DP14 max unconnected nodes | See "Tree Max Points Without Direct Connection" | - | -
DP37 weighted max unconnected nodes | " | " | `dp[i][0]` = same as above; `dp[i][1] = sum{dp[k][0] : k ∈ i} + value[i]` (add value instead of `1`)
